...
description: Use when user asks for 双色球/Double Color Ball prediction, 推荐号, 更新开奖数据, 回测, 归档分析, 补丁回灌, 生成预测网页(generate_html), GitHub Actions 自动化, 部署(deploy/Railway/VPS), 或直接运行 lottery-predictor-main 项目命令。
---
...
- 用户要生成预测展示网页或部署 GitHub Pages
- 用户要配置 GitHub Actions 自动化定时预测
...
- **核心池**：红球 24 球（默认/当前补丁值）+ 蓝球 10 球；使用 `22_red_cover_6_to_5` 旋转矩阵覆盖前22个池位输出 5 注
- **蓝球引擎**：`BlueBallEngine` 独立 7 维度分析，参数统一来自 `blue_params`
- **反共识辩论**：专家重新评估被排除的 11 球 → 合并重排名 → 可能晋升/降级；蓝球同理
...
- `team` 模式固定输出 `5` 注 `6+1`
- 核心管线：`_load_patches()` → `differential_learning()` → 8 专家提案 → `build_core_pool_snapshot()` → `BlueBallEngine.predict()` → `generate_rotation_matrix_tickets()` → `_archive_prediction()`
...
- 生成预测网页：`python generate_html.py`
...
- 网页展示：预测完成后运行 `python generate_html.py`，生成 `docs/index.html`，可由 GitHub Pages 托管
...
- 用户没有要预测、更新数据、回测、分析归档、生成网页、部署或运行本项目中的任何需求
- 用户只是问通用 GitHub Actions 语法，或讨论其他 Python 项目的部署，与本项目无关
- 用户只想编辑与本项目无关的文档或代码
